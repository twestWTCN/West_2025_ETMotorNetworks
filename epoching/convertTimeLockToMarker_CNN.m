function [exec_trl, hold_trl, react_time, testdata] = convertTimeLockToMarker_CNN(ftdata_tmp, trl, tranSamp, plotop, accBadFlag, searchwin, netExec, netHold, netQual)
% CONVERTTIMELOCKTOMARKER_CNN converts stimulus-locked trials to movement onset markers using a pretrained CNN.
% The function extracts relevant data, applies filters, and uses neural network models to predict movement and hold timings.
...
% Loop through each trial and analyze movement
while tri < size(trl, 1)
    ...
    if (trl(tri, 1) > 0) && (trl(tri, 2) <= size(ftdata_tmp.time{1}, 2))
        ...
        % Use predictor networks to predict movement and hold times
        preddata = [mdata; adata];
        preddata = preddata(1:6, :);
        qualPred = predict(netQual, preddata);
        execPred = predict(netExec, preddata);
        holdPred = predict(netHold, preddata);

        movlocal = [execPred, holdPred];
        x = tvec(1) + (movlocal ./ fsamp); % Convert to time

        % Plot prediction markers
        trtyp = {'r--', 'b--'};
        for i = 1:2
            plot([x(i) x(i)], XR1, trtyp{i});
        end

        % Save the markers based on quality prediction
        [~, kin] = max(qualPred);
        if kin == 1 || kin == 2
            movtime{1}(tri) = trl(tri, 1) + fix(movlocal(1));
            movtime{2}(tri) = trl(tri, 1) + fix(movlocal(2));
...
function [exec_trl, hold_trl, react_time, testdata] = convertTimeLockToMarker_CNN(ftdata_tmp, trl, tranSamp, plotop, accBadFlag, searchwin, netExec, netHold, netQual)
% CONVERTTIMELOCKTOMARKER_CNN converts stimulus-locked trials to movement onset markers using ML models.
% The function extracts relevant data, applies filters, and uses neural network models to predict movement and hold timings.
